 ...
 * rastadel.c
 * 
 * A tool to delete subtrees from rasta description files.
 *
 * Copyright (C) 2001 Oracle Corporation, Inc., Joel Becker
...
static gint delete_paths(RADContext *ctxt);
static gint process_screen(RADContext *ctxt, xmlNodePtr screens,
                           xmlNodePtr del_screen_parent);
...
{
    xmlNodePtr cur;
    gchar *test_id, *del_id;

    g_return_val_if_fail(ctxt != NULL, -EINVAL);
    g_return_val_if_fail(screens != NULL, -EINVAL);
    g_return_val_if_fail(del_screen != NULL, -EINVAL);

    del_id = xmlGetProp(del_screen, "ID");
    if (del_id == NULL)
        return(-EINVAL);
    else if (del_id[0] == '\0')
    {
    ...
    if ((cur == NULL) && (ctxt->verbose != FALSE))
    {
        fprintf(stderr,
                "rastadel: Screen \"%s\" does not exist - Ignored\n",
                del_id);
    }
 ...
 * static gint delete_paths(RADContext *ctxt)
 *
 * Removes the PATHDELETE elements from the main context.
 */
static gint delete_paths(RADContext *ctxt)
{
    gchar *path;
    ...
    {
        if ((cur->type == XML_ELEMENT_NODE) &&
            (xmlStrcmp(cur->name, "PATHDELETE") == 0))
        {
            ...
            if (target == NULL)
            {
                ...
                {
                    fprintf(stderr,
                            "rastadel: Path \"%s\" does not exist - Ignored\n",
                            path);
                }
            }
            else
...
static void print_usage(gint rc)
{
    ...
    fprintf(output,
            "Usage: rastadel [--file <system_file>] [--force] [--verbose] <deletion_file>\n");
    exit(rc);
}  /* print_usage() */
...
gint main(gint argc, gchar *argv[])
{
    ...
    if (semid < 0)
    {
        fprintf(stderr, "rastadel: Unable to acquire lock\n");
        return(semid);
    }
    ...
    {
        fprintf(stderr, "rastadel: Unable to parse file \"%s\"\n",
                ctxt->main_filename);
        rc = -ENOENT;
        goto out;
    }
    ...
    if (rc != 0)
    {
        fprintf(stderr, "rastadel: File \"%s\" is invalid\n",
                ctxt->main_filename);
        goto out;
    }
    ...
    rc = delete_screens(ctxt);
    if (rc != 0)
    {
        fprintf(stderr, "rastadel: Unable to delete screens\n");
        goto out;
    }
    
    rc = delete_paths(ctxt);
    if (rc != 0)
    {
        fprintf(stderr, "rastadel: Unable to delete paths\n");
        goto out;
    }
    ...
    {
        fprintf(stderr,
                "rastadel: Deletions create an invalid document\n");
        goto out;
    }

    rc = rad_save_file(ctxt);
    if (rc != 0)
        fprintf(stderr, "rastadel: Unable to save new copy of \"%s\"\n",
                ctxt->main_filename);
